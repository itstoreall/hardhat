  ...
  : describe("Funding", async () => {
      ...
      describe("withdraw", async () => {
        ...
        it("withdraw ETH from a single funder", async () => {
          const startFundBalance = await funding.provider.getBalance(
            funding.address
          );

          const startDeployerBalance = await funding.provider.getBalance(
            deployer
          );
          ...
          const endFundBalance = await funding.provider.getBalance(
            funding.address
          );

          const endDeployerBalance = await funding.provider.getBalance(
            deployer
          );

          // console.log("startFundBalance --->", startFundBalance.toString());
          // console.log(
          //   "startDeployerBalance --->",
        ...
        it("single cheaperWithdraw", async () => {
          const startFundBalance = await funding.provider.getBalance(
            funding.address
          );
          ...
          const transactionResponse = await funding.cheaperWithdraw();
          const transactionReceipt = await transactionResponse.wait(1);
        ...
        it("allows as to withdraw with multiple funders", async () => {
          ...
          for (let i = 0; i < 6; i += 1) {
            assert.equal(
              await funding.getAddressToAmountFunded(accounts[i].address),
              0
            );
          }
        });
